...
export default function Leaderboard({ exercise_question }: Props) {
    ...
    const sortedExam = React.useMemo(() => {
        return Object.values(
            groupBy(exercise_question.exams, it => it.user_id),
        )
    ...
    const dataColumns = [
        {
            header: 'Ranking',
            accessorFn: (it: ExamModel, index: number) => pagination.pageIndex * pagination.pageSize + index + 1,
            ...
            <div className="flex flex-col gap-3">
                <div className="flex justify-between">
                    <div className="font-5xl capitalize">
                        <p className="text-5xl text-[#3A63F5]">Ranking {exercise_question.name}</p>
                    </div>
                    <LinkButton
                        href={route("student.exam.show", { exercise_question: exercise_question.id }) as string}
                        colorCode="#3A63F5"
