...
export default function Create({ users, learningPackets }: Props) {
    ...
    let form = useForm<UserLearningPacketFormModel>({
        defaultValues: {
            user: params.user ? users.find(it => it.id.toString() === params.user) : {},
            learning_packet: params.learning_packet ? learningPackets.find(it => it.id.toString() === params.learning_packet) : {},
        },
    });
    ...
    return (
        ...
        >
            ...
            >
                <Form form={form} users={users} learningPackets={learningPackets} className="my-5 mx-2" />
