...
import { UserLearningPacketFormModel, UserLearningPacketModel } from '@/Models/UserLearningPacket';
...
export default function Create({ unregisteredUsers, learningPacket }: Props) {
	...
	function onSubmit(e: {}) {
		...
		router.post(route('user-learning-packet.store-many', [learningPacket.id]), { ...data, ...form.getValues() });
	}
				...
				<div className='grid grid-cols-3 gap-2 justify-center items-center'>
					<div className="flex flex-col gap-3  rounded-3xl shadow-2xl shadow-sky-400/50 p-3 bg-white">
						<FilterForm
							...
							users={unregisteredUsers.filter(it => !registered.find(user => user.id == it.id))}
					...
					<div className="flex flex-col gap-3  rounded-3xl shadow-2xl shadow-sky-400/50 p-3 bg-white">
						<FilterForm
							...
							users={learningPacket.users ? learningPacket.users.filter(it => !unregistered.find(user => user.id == it.id)) : []}
